...
- Initializes the RGB panel with a double framebuffer in PSRAM and a bounce
  buffer in internal SRAM, tearing-free
- Reads the GT911 touch over I2C and calibrates it to screen pixels
- Starts LVGL via `esp_lvgl_port` and shows a demo screen:
  - Title
  - Large tap counter in the center
  - "Tap me" button, increments the counter
  - Live uptime in the lower-left corner
  - Live touch coordinates in the lower-right corner
  - LVGL performance monitor overlay (FPS / CPU)

This verifies the whole display pipeline end to end. Custom UI widgets can be
added directly inside `build_ui()`.
...
└── main/
    ├── CMakeLists.txt             # component requires
    ├── idf_component.yml          # managed dependencies (LVGL, touch)
    └── main.c                     # panel + touch + LVGL + UI
```

`sdkconfig` is .gitignored and gets generated from `sdkconfig.defaults` on
the first `idf.py reconfigure`. When changing the defaults: delete
`sdkconfig` and regenerate.
...
1. **WiFi connect** — STA mode, reconnect logic, status indicator on screen
   (`esp_wifi`, `esp_event`, `nvs_flash`)
2. **NVS config** — persist WiFi credentials and user settings
3. **HTTPS / JSON API** — fetch external data, parse, display
   (`esp_http_client`, `cJSON`)
4. **Provisioning UI** — first-boot setup directly on the display
   (LVGL `lv_keyboard`, `lv_list`)
5. **Multi-screen dashboard** — tabs, custom widgets per data source
