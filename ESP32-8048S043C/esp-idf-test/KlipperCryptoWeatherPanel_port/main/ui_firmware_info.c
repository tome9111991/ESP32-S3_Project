...
// Zeigt Versions-Daten aus version.h und laesst Platz fuer spaetere OTA-UI
// (Check-Button, Status, Install-Button).
...
void ui_firmware_info_open(void)
{
    ...
    const struct { const char *label; const char *value; } rows[3] = {
    ...
    // Platzhalter-Bereich unter der Karte fuer spaetere OTA-UI
    // (Check-for-Update-Button + Status + Install-Button kommen hier rein).
